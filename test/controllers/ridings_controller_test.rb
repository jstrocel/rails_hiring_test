...
class RidingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create riding' do
    assert_difference('Riding.count', 0) do
      post ridings_url, params: { riding: { name: nil, province: @riding.province, riding_code: @riding.riding_code } }
    end

    assert_response :unprocessable_entity
  end

  test 'should not create riding if there are errors' do
    assert_difference('Riding.count') do
      post ridings_url,
           params: { riding: { name: @riding.name, province: @riding.province, riding_code: @riding.riding_code } }
    end
  ...
  test 'should update riding' do
    patch riding_url(@riding),
          params: { riding: { name: @riding.name, province: @riding.province, riding_code: @riding.riding_code } }
    assert_redirected_to riding_url(@riding)
  end

  test 'should not update riding if there are errors' do
    patch riding_url(@riding),
          params: { riding: { name: nil, province: @riding.province, riding_code: @riding.riding_code } }
    assert_response :unprocessable_entity
  end
